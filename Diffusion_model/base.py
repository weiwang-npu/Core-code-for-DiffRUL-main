...
class BaseDiffusion:
    ...
    def __init__(self, noise_steps=1000, beta_start=1e-4, beta_end=0.02, schedule_name="linear", device="cpu"):
        ...
        # Noise steps
        self.beta = self.prepare_noise_schedule(self.schedule_name).to(self.device)
        # Formula: α = 1 - β
    ...
    def noise_images(self, x, time):
        ...
        :param x: Input image, [B, L, feature_dim]
        :param time: Time, [B]
        :return: sqrt_alpha_hat * x + sqrt_one_minus_alpha_hat * Ɛ, a tensor of the same shape as the x tensor at time t
        """
        sqrt_alpha_hat = torch.sqrt(self.alpha_hat[time])[:, None, None]                # [B, 1, 1]
        sqrt_one_minus_alpha_hat = torch.sqrt(1 - self.alpha_hat[time])[:, None, None]  # [B, 1, 1]
